...
def _event_body(task: Task, *, include_tag: bool = True) -> dict[str, Any]:
    """Build the minimal event body for a task."""
    body: dict[str, Any] = {
        "summary": task.title,
    ...
    if include_tag:
        body["extendedProperties"] = {
            "private": {_JERICHO_TAG_KEY: task.id}
        }
    return body
...
class GoogleCalendarBackend:
    ...
    async def create_event(self, task: Task) -> str:
        ...
        body = _event_body(task, include_tag=True)
        result: dict[str, Any] = (
    ...
    async def update_event(self, task: Task, external_event_id: str) -> None:
        """Patch an existing event — only updates summary and dates."""
    ...
    async def delete_event(self, external_event_id: str) -> None:
        ...
        try:
            ...
            if exc.resp.status == 410:
                # Already deleted (410 Gone) — treat as success.
                return
            raise
