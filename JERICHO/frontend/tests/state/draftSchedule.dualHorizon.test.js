...
describe('draftSchedule dual horizon', () => {
  ...
  it('computes milestone slack with feasibility horizon beyond execution horizon', () => {
    ...
    const metrics = computeStressMetrics({
      fixture: {
        ...
        availability: { daysPerWeek: 7, specificDays: 'mon,tue,wed,thu,fri,sat,sun', maxBlocksPerDay: 3, routeMinutesDefault: 30 },
        milestones,
  ...
  it('is deterministic for emitted schedule and outside classification', () => {
    ...
    const first = buildDraftScheduleItems(state, cycleId, {
      ...
      contract: state.cyclesById[cycleId].goalContract,
      timeZone: 'UTC',
      defaults: { routeMinutes: 30, primaryDomain: 'FOCUS', todayKey: '2026-01-01', executionHorizonDays: 90, fullPlanMaxHorizonDays: 800 },
      captureStats: (stats) => {
        firstStats = stats;
      }
    });
    ...
    expect(secondStats.outsideExecutionHorizonEstimateMinTotal).toBe(firstStats.outsideExecutionHorizonEstimateMinTotal);
  });
});
